    ...
    <>
      ...
      {hovered && pos && ReactDOM.createPortal(
        <div
          ...
          className="w-[320px] max-h-[360px] overflow-auto dark-scroll rounded-2xl border border-[#242424] bg-[#0E0E0E] shadow-[0_20px_60px_rgba(0,0,0,0.6)] p-3"
        >
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {items.map((f) => (
                <div key={f.id} className="bg-[#141414] border border-[#222] rounded-xl px-3 py-2">
...
export function FeedbackPanel({
  ...
  return ReactDOM.createPortal(
    ...
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="bg-[#0C0C0C]/95 border border-white/10 backdrop-blur-2xl rounded-t-3xl sm:rounded-3xl w-full sm:max-w-[560px] shadow-[0_20px_80px_rgba(0,0,0,0.6)] flex flex-col max-h-[90vh]"
      >
        {/* HEADER */}
                      ...
                      {mine && (
                        <div className="flex gap-3 mt-2">
                          <button
                            ...
                            className="text-[11px] text-zinc-500 hover:text-[#D6B36A] transition disabled:opacity-40"
                          >
                            Edit
                          </button>
                          <button
                            onClick={() => remove(f.id)}
                            disabled={deletingId === f.id}
                            className="text-[11px] text-zinc-500 hover:text-red-400 transition disabled:opacity-40 disabled:cursor-wait"
                          >
